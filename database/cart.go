...
import (
	...
	"github.com/Maryu47/ecommerce-bill/models"
...
var (
	...
	ErrCantRemoveItemCart = errors.New("cannot remove this product to cart")
...
func InstantBuyer(ctx context.Context, prodCollection, userCollection *mongo.Collection, productID primitive.ObjectID, userID string) error {
	...
	filter := bson.D{primitive.E{Key: "_id", Value: id}}
	updated := bson.D{{Key: "$push", Value: bson.D{primitive.E{Key: "orders", Value: order_detail}}}}
	_, err = userCollection.UpdateOne(ctx, filter, updated)
	if err != nil {
		log.Println(err)
	}

	filter2 := bson.D{primitive.E{Key: "_id", Value: id}}
	updated2 := bson.M{"$push": bson.M{"orders.$[].order_list": product_details}}

	_, err = userCollection.UpdateOne(ctx, filter2, updated2)
	if err != nil {
		log.Println(err)
	}
	return nil
